...
if (Meteor.isServer) {
	...
	checkBank = function(doc) {
	 ...
	 UsersDatas.before.update(function(uid,doc, f, m) {
		if (doc.firstname != m['$set'].firstname ||
				doc.name != m['$set'].name ||
				doc.birthdate.getTime() != m['$set'].birthdate.getTime())
			upsertMangoUser(doc.userId);
		if (m['$set'].iban && m['$set'].bic &&
			 (m['$set'].iban != doc.iban ||
				m['$set'].bic != doc.bic)) {
			m['$set'].bankComplete = upsertMangoBank(doc.userId);
			//console.log('before.update - upsertMangoBank');
		}
	 });
...
/*
		if (!checkBank(this.previous) && checkBank(doc))
		...
		else if (checkBank(this.previous) && checkBank(doc) && !doc.bankComplete)
			UsersDatas.update(doc, {$set: {bankComplete: true}});
*/
	});
}
