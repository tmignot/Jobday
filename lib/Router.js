...
var fun = _.debounce(function() {
	if (!Session.get('setMissionType'))
		Session.set('searchMission_type', 'all');
	else
		Session.set('setMissionType', false);
	
	if (!Session.get('setMissionPage'))
		Session.set('searchMission_page', 0);
	else
		Session.set('setMissionPage', false);
}, 1000, true);
...
Router.route('/searchMission', {
	...
	waitOn: function() {
		...
		return [
			Meteor.subscribe('AdvertsPage', this.params.query),
			Meteor.subscribe('UserNotification', Meteor.userId()),
			Meteor.subscribe('userData', [Meteor.userId()]),
			Meteor.subscribe('Images')
		];
