...
if (Meteor.isServer) {
	checkProfile = function(doc) {
		if (doc.phone && doc.address && doc.address.street && doc.address.zipcode && doc.address.city && doc.name && ((!doc.society && doc.firstname)||doc.society))
			return true;
		return false;
	};
	UsersDatas.after.update(function(uid,doc) {
		console.log(checkProfile(this.previous), checkProfile(doc));
