...
b64toJSON = function(b64) {
	if (b64) {
		var str = String.fromCharCode.apply(null, Base64.decode(b64));
...
Router.route('/searchMission', {
	...
	waitOn: function() {
		...
		Session.set('searchMissionKeyword', undefined);

		var p = this.params.query;

		var sort = p.sort ? b64toJSON(p.sort) : {createdAt: -1};
		if (sort.createdAt == -1)
			Session.set('searchMission_sort', 'date_desc');
		else if (sort.startDate)
			Session.set('searchMission_sort', 'date_asc');
		else if (sort.budget == 1)
			Session.set('searchMission_sort', 'price_asc');
		else if (sort.budget == -1)
			Session.set('searchMission_sort', 'price_desc');

		var filters = p.filters ? b64toJSON(p.filters) : {};
		if (filters.category != undefined)
			Session.set('currentCategory', filters.category);
		if (filters.subcategory != undefined)
			Session.set('currentSubcategory', filters.subcategory);
		if (filters.title && filters.title['$regex']) {
			var t = filters.title['$regex'];
			Session.set('searchMissionNeed', t.substr(3, t.length - 6));
		}
		if (filters.description && filters.description['$regex']) {
			var d = filters.description['$regex'];
			Session.set('searchMissionKeyword', d.substr(3, d.length - 6));
		}
		if (filters['$or'] && filters['$or'][1] && filters['$or'][1]['address.city'] &&
			  filters['$or'][1]['address.city'] && filters['$or'][1]['address.city']['$regex']) {
			var c = filters['$or'][1]['address.city']['$regex'];
			Session.set('searchMissionLocal', c.substr(3, c.length - 6));
		}
		...
		if (!page || page < 0)
			page = 0;
		Session.set('searchMission_page', page);

		return [
			Meteor.subscribe('AdvertsPage', {page: page, filters: filters, sort: sort}),
			Meteor.subscribe('UserNotification', Meteor.userId()),
			Meteor.subscribe('userData', [Meteor.userId()]),
			Meteor.subscribe('Images')
		];
