...
UsersDatas.deny({
	...
	update: function(uid, doc, fieldNames) {
		if (doc.userId === uid) {
			var forbidden = ['adverts', 'notes', 'sponsor', 'badges', 'userId', 'devices', 'profileComplete'];
...
if (Meteor.isServer) {
	...
	checkProfile = function(doc) {
		if (doc.birthdate && doc.phone && doc.address && doc.address.street && doc.address.zipcode && doc.address.city && doc.name && ((!doc.society && doc.firstname)||doc.society))
			return true;
		return false;
	};
